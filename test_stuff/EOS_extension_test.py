...
def extend_EOS_polytrop(ns, starts, Ks, gammas, n_step=None, size=None):
    """
    Function to extend the EOS by using linear segments in speed of sound
...
def extend(small_EOS, nsamp, size=None, rho_step=1e-3, rho_max=10, ext_type=None, max_gamma=3):
    """
    ...
    if ext_type == None:
        ...
        EOS_ex1 = extend_cs(ns, starts, cs2_func, rho_step, size)
        EOS_tot1 = stitch_EOS(small_EOS, EOS_ex1)
        # extend EOS with linear P interpolation
        EOS_ex2 = extend_Plin(ns, starts, cs, rho_step, size)
        EOS_tot2 = stitch_EOS(small_EOS, EOS_ex2)
        # sample parameters
        gammas, Ks = sample_polytrop(nsamp, ns, p[-1], max_gamma)

        # extend EOS
        EOS_ex3, causality = extend_EOS_polytrop(ns, starts, Ks, gammas, rho_step, size)
    ...
    if ext_type == 'cs' or ext_type == 1:
        ...
        # extend EOS
        EOS_ex = extend_cs(ns, starts, cs2_func, rho_step, size)
        ...
        # extend EOS
        EOS_ex = extend_Plin(ns, starts, cs, rho_step, size)
        EOS_tot = stitch_EOS(small_EOS, EOS_ex)
        ...
        # extend EOS
        EOS_ex, causality = extend_EOS_polytrop(ns, starts, Ks, gammas, rho_step, size)
        EOS_tot = stitch_EOS(small_EOS, EOS_ex)
